...
pub fn prove_circuit_assignment_keccak(
    values: &[QM31],
    preprocessed_circuit: &PreprocessedCircuit,
    base_column_pool: &BaseColumnPool<SimdBackend>,
) -> CircuitProofKeccak {
    ...
    let store_polynomials_coefficients = true;
    let preprocessed_tree = CommitmentTreeProver::<SimdBackend, KeccakMerkleChannel>::new(
        preprocessed_trace_polys,
        pcs_config.fri_config.log_blowup_factor,
        &twiddles,
        store_polynomials_coefficients,
    ...
    prove_circuit_with_precompute_keccak(
        base_column_pool,
        &twiddles,
        preprocessed_circuit,
        MaybeOwned::Owned(preprocessed_tree),
...
pub fn prove_circuit_with_precompute_keccak<'a>(
    base_column_pool: &BaseColumnPool<SimdBackend>,
    twiddles: &TwiddleTree<SimdBackend>,
    preprocessed_circuit: &PreprocessedCircuit,
    preprocessed_tree: MaybeOwned<'a, CommitmentTreeProver<SimdBackend, KeccakMerkleChannel>>,
    values: &[QM31],
    pcs_config: PcsConfig,
) -> CircuitProofKeccak {
    let PreprocessedCircuit { preprocessed_trace, params } = preprocessed_circuit;
    ...
    pcs_config.mix_into(channel);
    let mut commitment_scheme =
        CommitmentSchemeProver::<SimdBackend, KeccakMerkleChannel>::with_memory_pool(
            pcs_config,
            twiddles,
            base_column_pool,
        );
    ...
    // Base trace.
    let mut tree_builder = commitment_scheme.tree_builder();
    let (claim, interaction_generator) = write_trace_keccak(
        values,
        preprocessed_trace.clone(),
        output_addresses,
        &mut tree_builder,
    ...
    // Interaction trace.
    let mut tree_builder = commitment_scheme.tree_builder();
    let interaction_claim = write_interaction_trace_keccak(
        &claim,
        interaction_generator,
        &mut tree_builder,
        &interaction_elements,
...
pub fn preprare_circuit_proof_for_circuit_verifier_keccak(
    circuit_proof: CircuitProofKeccak,
    proof_config: &ProofConfig,
) -> (Proof<QM31>, CircuitPublicData) {
...
pub fn verify_stwo_proof_keccak(preprocessed_circuit: &PreprocessedCircuit, circuit_proof: CircuitProofKeccak) {
    let CircuitProofKeccak {
        pcs_config,
        claim,
        interaction_pow_nonce,
